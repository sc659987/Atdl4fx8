...
public class TimeSpinner extends Spinner<ZonedDateTime> {
	...
	public enum Mode {
		HOURS {
			@Override
			ZonedDateTime increment(ZonedDateTime time, int steps) {
		...
		abstract ZonedDateTime increment(ZonedDateTime time, int steps);

		abstract void select(TimeSpinner spinner);

		ZonedDateTime decrement(ZonedDateTime time, int steps) {
	...
	StringConverter<ZonedDateTime> localTimeConverter = new StringConverter<ZonedDateTime>() {
		...
		public ZonedDateTime fromString(String string) {
			...
			LocalTime localTime = LocalTime.of((totalSeconds / 3600) % 24, (totalSeconds / 60) % 60, seconds % 60);
			return ZonedDateTime.of(LocalDate.now(), localTime, time.getZone());
		}
	...
	TextFormatter<ZonedDateTime> textFormatter = new TextFormatter<ZonedDateTime>(localTimeConverter, ZonedDateTime.now(), c -> {
	...
	public TimeSpinner(ZonedDateTime time) {
		setEditable(true);
		this.time = time;
